...
describe('POST /api/import-costaleros', () => {
  ...
  it('should return 500 when RPC fails', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      status: 200,
      json: async () => [
        { id: 1, nombre: 'Juan', apellidos: 'García', apodo: 'El Rubio', trabajadera: 3, email: 'juan@test.com' },
      ],
    })

    mockAdmin.rpc.mockResolvedValue({ data: null, error: { message: 'DB transaction failed' } })
  ...
  it('should perform full sync successfully', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: async () => [
        { id: 1, nombre: 'Juan', apellidos: 'García', apodo: 'El Rubio', trabajadera: 3, email: 'juan@test.com' },
        { id: 2, nombre: 'Pedro', apellidos: 'López', apodo: '', trabajadera: 5, email: '' },
      ],
    })
