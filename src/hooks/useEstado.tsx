...
export type { ActivePage, ActiveSheet, PasoDB, DatosPerfil, Trabajadera, RolCode, PinState, SwapState, CellTarget, CensusTarget, Temporada } from './useEstadoTypes'
...
export function useEstado(): EstadoCtx {
  ...
  return {
    activePage, setActivePage: uiStore.getState().setActivePage,
    activeSheet, openSheet: uiStore.getState().openSheet, closeSheet: uiStore.getState().closeSheet,
    tema, toggleTema: uiStore.getState().toggleTema,
    swapSel, setSwapSel: uiStore.getState().setSwapSel,
    cellTarget, setCellTarget: uiStore.getState().setCellTarget,
    bancoTarget, setBancoTarget: uiStore.getState().setBancoTarget,
    censusTarget, setCensusTarget: uiStore.getState().setCensusTarget,
    openEqs, toggleEq: uiStore.getState().toggleEq,
    pasos, pid, setPid: projectStore.getState().setPid, S, nombrePaso, nombreCuadrilla,
    censusHeights, refetchPasos, vaciarCenso,
    temporadas, activeTemporadaId, setActiveTemporadaId: temporadaStore.getState().setActiveTemporadaId,
    setNombre: trabajaderaStore.getState().setNombre, addCost, delCost: trabajaderaStore.getState().delCost,
    ...
    delPlan: planStore.getState().delPlan, cargarPlanEnTrabajadera: planStore.getState().cargarPlanEnTrabajadera,
  }
}
