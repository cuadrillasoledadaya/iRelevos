...
import type {
  ...
  NewCensusEntry, NewPasoForm, NewTempForm, CensusEditForm,
...
export function useAdminMutations(
  ...
  setUsuarios: React.Dispatch<React.SetStateAction<import('@/hooks/useAuth').Profile[]>>,
  ...
  const [newPaso, setNewPaso] = useState<NewPasoForm>({
    nombre_paso: '', nombre_cuadrilla: '', num_trabajaderas: 6,
  })
  const [newTemp, setNewTemp] = useState<NewTempForm>({
    nombre: '', clonarCenso: true, clonarPasos: true, sourceTempId: '',
  })
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editForm, setEditForm] = useState<CensusEditForm>({
    email: '', nombre: '', apellidos: '', apodo: '', telefono: '',
    trabajadera: 0, altura: 0,
  })
  ...
  const editarPerfil = useCallback(async (uid: string, usuarios: import('@/hooks/useAuth').Profile[]) => {
  ...
  const addPaso = useCallback(async (e: React.FormEvent) => {
    e.preventDefault()
    ...
    const { error } = await supabase
      ...
      .insert([{
        ...
        content: {
          banco: [],
          trabajaderas: Array.from({ length: newPaso.num_trabajaderas }, (_, i) => ({
            id: i + 1,
            nombres: [],
            roles: [],
            salidas: 1,
            ...
            plan: null, obj: {}, analisis: null,
            pinned: null, puntuaciones: {}, tramosClaves: [],
          })),
        },
  ...
  const eliminarPaso = useCallback(async (id: string) => {
  ...
  const addToCensus = useCallback(async (e: React.FormEvent) => {
    e.preventDefault()
  ...
  const syncCostaleroToProject = useCallback(async (proyectoId: string, trabajaderaId: number, displayName: string) => {
    const { data: { session } } = await supabase.auth.getSession()
    ...
    const content = proj.content as { trabajaderas: { id: number; nombres: string[]; roles?: { pri: string; sec: string }[] }[] }
  ...
  const deleteFromCensus = useCallback(async (id: string) => {
  ...
  const syncTodoCenso = useCallback(async (proyectoId: string) => {
    setSaving(true)
  ...
  const syncCensoDesdeProyecto = useCallback(async (proyectoId: string) => {
  ...
  const eliminarTemporada = useCallback(async (id: string, onDeleted?: () => void) => {
    ...
    try {
      ...
      if (projects && projects.length > 0) {
        ...
        if (cErr) {
          ...
          // eslint-disable-next-line no-console
          console.log(`Censo borrado (vía proyectos): ${cDel?.length || 0} registros`)
        }
      }
  ...
  const crearTemporada = useCallback(async (
    form: NewTempForm,
    onSuccess?: () => void,
  ) => {
    ...
    try {
      ...
      if (form.sourceTempId) {
        if (form.clonarPasos) {
          const { data: oldP } = await supabase.from('proyectos').select('*').eq('temporada_id', form.sourceTempId)
        ...
        if (form.clonarCenso) {
          const { data: oldC, error: cFetchErr } = await supabase.from('census').select('*').eq('temporada_id', form.sourceTempId)
          if (cFetchErr) {
            // eslint-disable-next-line no-console
  ...
  return {
    ...
    importPid, setImportPid,
    importPreview, setImportPreview,

    // Acciones
    eliminarUsuario, cambiarRol, editarPerfil,
    addPaso, eliminarPaso,
    addToCensus, deleteFromCensus, saveEdit,
    syncCostaleroToProject, syncTodoCenso, syncCensoDesdeProyecto,
    reconstruirCensoCompleto,
    fetchFromICuadrilla, ejecutarImportacion, sincronizacionTotal,
    eliminarTemporada, crearTemporada,
  }
}
