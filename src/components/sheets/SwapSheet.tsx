...
export default function SwapSheet() {
	...
	const confirmarSwap = planStore.getState().confirmarSwap;
	...
	if (!swapSel || !("a" in swapSel) || !("b" in swapSel)) {
		return (
			<>
				<div
					className={`bso${activeSheet === "swap" ? " open" : ""}`}
					onClick={closeSheet}
				/>
	...
	function validarRol(
		...
		return {
			ok: false,
			msg: `No tiene ${rolLabel(rolNuevo, t.id)} en su repertorio`,
		};
	}

	const rolActualA = a.esDentro
		? a.posIdx !== null
			? estructura[a.posIdx]
			: null
		: null;
	const rolActualB = b.esDentro
		? b.posIdx !== null
			? estructura[b.posIdx]
			: null
		: null;
	...
	const validMsg = compatible
		...
		: `⚠ Incompatibilidad de roles: ${[
				!valA.ok
					? `${nameAt(t, a.ci)} no es ${rolLabel(rolNuevoA!, t.id)}`
					: "",
				!valB.ok
					? `${nameAt(t, b.ci)} no es ${rolLabel(rolNuevoB!, t.id)}`
					: "",
			]
				.filter(Boolean)
				.join(" · ")}`;
	...
	function SwapCard({
		...
		ci: number;
		rolActual: RolCode | null;
		rolNuevo: RolCode | null;
		val: { ok: boolean; msg: string };
		esDentro: boolean;
		rolClass: string;
	}) {
		...
		return (
			<div className={`swap-card ${rolClass}`}>
				...
				<div className="swap-card-rol">
					{esDentro ? (rolActual ?? "Dentro") : "Fuera"}
				</div>
				<div className={`swap-card-new ${val.ok ? "ok" : "err"}`}>
					{rolNuevo ? `→ ${rolLabel(rolNuevo, t.id)}` : "→ Fuera"}
				</div>
				<div
					className={`swap-card-new ${val.ok ? "ok" : "err"}`}
			...
			<div className={`bss${isOpen ? " open" : ""}`}>
				...
				<div className="bs-hdr">
					<span className="bs-title">
						🔄 {ambosD ? "Cambio de posición" : "Cambio dentro/fuera"} — T{t.id}{" "}
						· Tramo {a.ti + 1}
					</span>
					<button
						...
						{!compatible && (
							<label className="flex items-center gap-2 mt-2 p-2 bg-red-900/20 border border-red-500/30 rounded cursor-pointer">
								<input
									...
									className="w-4 h-4"
								/>
								<span className="text-[0.7rem] font-bold text-red-100">
									Confirmar cambio manual (fuera de rol)
								</span>
							</label>
						)}
					</div>
