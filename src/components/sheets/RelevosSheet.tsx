...
import { uiStore, projectStore } from "@/stores";
import {
	exportarRelevos,
	exportarRelevosIndividual,
	exportarPDF,
	exportarRelevosMultiplesItems,
	exportarPDFMasivoTodas,
} from "@/lib/exportar";
...
export default function RelevosSheet() {
	...
	const conPlan = S.trabajaderas.filter(
		(
			...
			plan: NonNullable<Trabajadera["plan"]>;
			analisis: NonNullable<Trabajadera["analisis"]>;
		} => !!(t.plan && t.analisis),
	);
	...
	function handleGenerarSeleccionadas() {
		const trabs = conPlan.filter(
			(t: (typeof conPlan)[number]): t is (typeof conPlan)[number] =>
				seleccionadas.has(t.id),
		);
		if (!trabs.length) {
			alert("⚠ Selecciona al menos una trabajadera.");
			return;
		}
		closeSheet();
	...
	function handleGenerarIndividual() {
		if (!tidIndividual) {
			alert("⚠ Selecciona una trabajadera.");
			return;
		}
		if (ciIndividual === "") {
			alert("⚠ Selecciona un costalero.");
			return;
		}
		const t = conPlan.find(
			(x: (typeof conPlan)[number]) => x.id === tidIndividual,
		);
		if (!t) return;
	...
	function handleGenerarTodos() {
		...
		if (
			confirm(
				`¿Generar las tablas individuales de los ${t.nombres.length} costaleros de la Trabajadera ${t.id}?`,
			)
		) {
			closeSheet();
	...
	const trabajaderaIndividual = conPlan.find(
		(x: (typeof conPlan)[number]): x is (typeof conPlan)[number] =>
			x.id === tidIndividual,
	);
						...
						>
							...
							<div>
								...
								>
									{conPlan.map((t: Trabajadera) => (
										...
										>
											...
											/>
											<span style={{ flex: 1 }}>
												<div style={{ fontWeight: 600, color: "var(--cre)" }}>
													Trabajadera {t.id}
												</div>
												<div
													style={{ fontSize: ".75rem", color: "var(--oro-o)" }}
												>
													{t.nombres.length} costaleros · {t.tramos.length}{" "}
													tramos
												</div>
											</span>
											{t.analisis?.rep.length === 0 ? (
												<span
													style={{ color: "var(--ok-tx)", fontWeight: 700 }}
												>
													✓
												</span>
											) : (
												<span
													style={{ color: "var(--warn-oro)", fontWeight: 700 }}
												>
													⚠
												</span>
											)}
								...
								<button
									...
									onClick={handleGenerarSeleccionadas}
								>
									📋 Generar (Opción 1)
								</button>
							</div>
							...
							>
								...
								<div style={{ marginBottom: ".6rem" }}>
									...
									<select
										...
										value={tidIndividual}
										onChange={(e) => {
											setTidIndividual(e.target.value ? +e.target.value : "");
											setCiIndividual("");
										}}
										style={{ height: "42px" }}
									>
										...
										{conPlan.map((t: Trabajadera) => (
											<option key={t.id} value={t.id}>
												Trabajadera {t.id} ({t.nombres.length} costaleros)
											</option>
										))}
									</select>
								</div>
								{trabajaderaIndividual && (
									<div style={{ marginBottom: ".6rem" }}>
										...
										>
											<option value="">-- Selecciona --</option>
											{trabajaderaIndividual.nombres.map(
												(nombre: string, i: number) => (
							...
							>
								...
								>
									Genera una hoja horizontal (PDF) con la cuadrícula completa de
									relevos para todas las trabajaderas con plan.
								</p>
								<button
									className="btn btn-oro w100"
									onClick={() => {
							...
							>
								...
								>
									Genera <strong>una página por costalero</strong> de todas las
									trabajaderas. Cada uno ve su nombre y cuándo le toca. Mandá el
									PDF entero al grupo y que cada uno busque su hoja.
								</p>
								<div
									style={{
								...
								>
									...
									{conPlan.reduce(
										(acc, t) =>
											acc +
											t.nombres.filter((_, i) => !t.bajas?.includes(i)).length,
										0,
									)}{" "}
									páginas en total
								</div>
							...
							<button className="btn btn-ghost w100" onClick={closeSheet}>
								Cancelar
							</button>
						</div>
					)}
