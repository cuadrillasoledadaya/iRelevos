...
export default function AdminPage() {
  ...
  useEffect(() => {
    m.setNewTemp(prev => ({ ...prev, sourceTempId: activeTemporadaId || '' }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTemporadaId])
  ...
  const handleToggleSelected = useCallback((idx: number) => {
  ...
  const handleToggleAllSelected = useCallback(() => {
    ...
    m.setImportPreview(m.importPreview.map(c => ({ ...c, selected: !allSelected })))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [m.importPreview, m.setImportPreview])

  const handleStartEdit = useCallback((entry: import('@/components/admin/types').CensusEntry) => {
  ...
  const handleEliminarTemporada = useCallback((id: string) => {
    ...
    <div className="p-4 flex flex-col gap-6 pb-20">
      ...
      <div className="text-center max-w-md mx-auto w-full">
        ...
        <p className="text-[var(--cre-o)] text-[0.6rem] uppercase font-bold tracking-[0.2em] mb-6">
          Temporada: <span className="text-[var(--oro)]">
            {temporadas.find(t => t.id === activeTemporadaId)?.nombre || 'Cargando...'}
          </span>
        </p>
        <div className="flex bg-black/30 p-1.5 rounded-2xl mb-8 border border-[var(--oro)]/10 shadow-inner">
          {(['usuarios', 'censo', 'pasos', 'temporadas'] as AdminTab[]).map(tab => (
            <button
              key={tab}
