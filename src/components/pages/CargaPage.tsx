		...
		return (
			<>
				...
				<div className="alert warn">
					⚠ Aún no hay rotaciones calculadas. Ve a la pestaña &quot;Plan&quot; y
					calcula las rotaciones primero.
				</div>
	...
	return (
		<div className={`card ${isOpen ? "open" : ""} res-trab`}>
			...
			<div className="trab-body fc g3">
				...
				>
					<div style={{ fontWeight: 700, color: "var(--oro)" }}>
						Objetivo matemático:
					</div>
					{objMsg} (Total {plazas} plazas de descanso en {t.tramos.length}{" "}
					tramos)
				</div>
						...
						<tbody>
							{t.nombres.map((_: string, ci: number) => {
								...
								for (let ti = 1; ti < t.plan!.length; ti++) {
									if (
										t.plan![ti].fuera.includes(ci) &&
										t.plan![ti - 1].fuera.includes(ci)
									)
										cV++;
								}
								...
								return (
									<tr key={ci}>
										...
										<td>{t.obj![ci]}</td>
										<td
											className={`cinzel toro ${an.conteo[ci] === t.obj![ci] ? "tok" : "terr"}`}
											style={{ fontWeight: 700 }}
										>
											{an.conteo[ci]}
										</td>
										<td className={isRep ? "td-n u" : "td-n"}>
											{isRep ? "Sí" : "—"}
										</td>
										<td className={cV > 0 ? "td-n u" : "td-n"}>
											{cV > 0 ? cV : "—"}
										</td>
									</tr>
								);
...
function HeightAnalysis({
	...
	> = {
		...
		FIJ: {
			bg: "rgba(180,100,0,0.35)",
			border: "rgba(230,140,20,0.7)",
			text: "#ffe082",
		},
		COR: {
			bg: "rgba(26,92,42,0.35)",
	...
	const data = trabajaderas.map((t) => {
		...
		const entries = t.nombres.map((n: string, idx: number) => {
			...
			return {
				...
				baja: isBaja(idx),
				altura:
					!isBaja(idx) && censusHeights[n.trim()]
						? censusHeights[n.trim()]
						: null,
				rol,
		...
		const conAltura = activos
			.filter((e) => e.altura !== null)
			.map((e) => e.altura as number);
		const maxH = conAltura.length > 0 ? Math.max(...conAltura) : null;
		const minH = conAltura.length > 0 ? Math.min(...conAltura) : null;
		const media =
			conAltura.length > 0
				? conAltura.reduce((a, b) => a + b, 0) / conAltura.length
				: null;
		...
		if (!sinDatos) {
			...
			for (let i = 0; i < conH.length; i++) {
				for (let j = i + 1; j < conH.length; j++) {
					...
					if (
						a.rolPrio < b.rolPrio &&
						(a.altura as number) < (b.altura as number)
					)
						inversiones++;
					if (
						b.rolPrio < a.rolPrio &&
						(b.altura as number) < (a.altura as number)
					)
						inversiones++;
				}
			}
	...
	return (
		<div style={{ marginBottom: "1.5rem" }}>
			...
			{criticas.length > 0 && (
				...
				>
					<strong style={{ color: "#ef4444" }}>✗ Corrección urgente:</strong>{" "}
					Trabajadera{criticas.length > 1 ? "s" : ""}{" "}
					<strong>{criticas.map((d) => d.t.id).join(", ")}</strong> — Δ &gt; 2.5
					cm.
				</div>
			)}
			{conInversiones.length > 0 && (
				...
				>
					<strong style={{ color: "#eab308" }}>↕ Inversión de roles:</strong>{" "}
					Trabajadera{conInversiones.length > 1 ? "s" : ""}{" "}
					<strong>{conInversiones.map((d) => d.t.id).join(", ")}</strong> — un
					costalero de menor rango es más alto que uno de mayor rango.
				</div>
			)}
			...
			>
				{[
					...
					>
						...
						<span style={{ fontSize: "0.58rem", color: "var(--cre-o)" }}>
							{hint}
						</span>
					</div>
				))}
								...
								>
									<div
										style={{
											...
											borderRadius: "50%",
											background:
												"linear-gradient(135deg, var(--oro-o), var(--oro))",
											display: "flex",
											alignItems: "center",
									...
									<div style={{ flex: 1 }}>
										...
										{!sinDatos && media && (
											...
											>
												Media{" "}
												<strong style={{ color: "var(--oro)" }}>
													{media.toFixed(1)} cm
												</strong>
												{" · "}Rango{" "}
												<strong style={{ color: "var(--cre)" }}>
										...
										{sinDatos && (
											...
											>
												Sin datos de altura en el censo
											</div>
										)}
									</div>
									{delta !== null && (
										<div
											style={{
												...
												borderRadius: "6px",
												background: borderColor.replace("0.35", "0.15"),
												border: `1px solid ${borderColor}`,
											}}
										>
											<span
								...
								{!sinDatos && (
									...
									>
										{vShape.map((e) => {
											...
											const rc =
												ROL_COLOR[e.rol] ?? ROL_COLOR[rolKey] ?? ROL_COLOR.COR;
											return (
												<div
													key={e.idx}
													style={{
