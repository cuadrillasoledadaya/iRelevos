				...
				{t.tramos.map((nombre: string, ti: number) => {
					...
					return (
						<div key={ti} className="mb3">
							...
							>
								<span className="cinzel toro mr2" style={{ opacity: 0.8 }}>
									T{ti + 1}
								</span>{" "}
								{nombre}
							</div>
							...
							<div
								className="paso-row"
								style={{
									opacity: localSwapSel && !isSwapTargetRow ? 0.3 : 1,
												...
												{ci !== null ? (
													...
													>
														{!esRolHabitual(t, ci, rol) && (
															<div
																className="absolute -top-1 -right-1 w-2 h-2 bg-red-600 border border-white"
																title="Fuera de rol habitual"
															/>
														)}
														{tClass === "sel-swap" && (
														...
														<span className="paso-pill-star inline-block text-[0.65rem] leading-none mb-0.5">
															{estrella}
														</span>
														<span className="paso-pill-name block text-[0.75rem] whitespace-nowrap overflow-hidden text-ellipsis max-w-[56px]">
