...
interface PasosTabProps {
  ...
  loading: boolean
  saving: boolean
  newPaso: { nombre_paso: string; nombre_cuadrilla: string; num_trabajaderas: number }
  onNewPasoChange: (v: { nombre_paso: string; nombre_cuadrilla: string; num_trabajaderas: number }) => void
...
export default function PasosTab({
  pasos, loading, saving,
  newPaso, onNewPasoChange, onAddPaso,
  onEliminarPaso, onSyncTodoCenso, onSyncCensoDesdeProyecto,
}: PasosTabProps) {
  return (
    <div className="flex flex-col gap-6">
      <form onSubmit={onAddPaso} className="bg-[var(--card)] border border-[var(--oro)]/30 p-4 rounded-lg flex flex-col gap-3">
        <h3 className="cinzel text-[var(--oro)] text-sm font-bold">Crear Nuevo Paso</h3>
        <input className="inp" placeholder="Nombre del Paso (ej: Virgen de la Paz)" required
          value={newPaso.nombre_paso}
          onChange={e => onNewPasoChange({ ...newPaso, nombre_paso: e.target.value })} />
        <input className="inp" placeholder="Nombre de la Cuadrilla" required
          value={newPaso.nombre_cuadrilla}
          onChange={e => onNewPasoChange({ ...newPaso, nombre_cuadrilla: e.target.value })} />
        <div className="flex aic jb gap-2">
          ...
          <input className="inp w-20" type="number" min="1" max="15" required
            value={newPaso.num_trabajaderas}
            onChange={e => onNewPasoChange({ ...newPaso, num_trabajaderas: parseInt(e.target.value) })} />
        </div>
