 ...
 */
export function createTrabajadera(overrides?: Partial<Trabajadera>): Trabajadera {
  const id = overrides?.id ?? 1
  const nombres = overrides?.nombres ?? [
    'Costalero A', 'Costalero B', 'Costalero C',
    'Costalero D', 'Costalero E', 'Costalero F',
  ]
  const tramos = overrides?.tramos ?? ['Tramo 1', 'Tramo 2', 'Tramo 3']

  const defaultRoles: Rol[] = nombres.map((_, i) => {
    const codes: Array<Rol['pri']> = ['PAT', 'COS', 'FIJ', 'COR']
    return { pri: codes[i % codes.length], sec: 'COS' }
  })
 ...
 */
export function createTrabajaderaValida(id = 1, numTramos = 3, numCostaleros = 6): Trabajadera {
  const nombres = Array.from({ length: numCostaleros }, (_, i) => `Costalero ${i + 1}`)
 ...
 */
export function createTrabajaderaConProblemas(id = 2, numTramos = 3, numCostaleros = 6): Trabajadera {
  const nombres = Array.from({ length: numCostaleros }, (_, i) => `Costalero ${i + 1}`)
  const tramos = Array.from({ length: numTramos }, (_, i) => `Tramo ${i + 1}`)
  ...
  return createTrabajadera({
    ...
    analisis: {
      conteo: Object.fromEntries(nombres.map((_, i) => [
        i,
